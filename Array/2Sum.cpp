...
public:
	vector<int> twoSum(vector<int> &numbers, int target) {
		...
		for (int i = 0; i < numbers.size(); i++) {
			int key_num = target - numbers[i];
			if (myMap.find(key_num) != myMap.end()) {
				int index = myMap[key_num];
				if (index == i) {//两个坐标要求不同
					continue;
				}else if(index < i) {
					ret.push_back(index);
...
我的思路:用O(n)将数组元素存入HashMap中,然后在遍历hashMap,使用O(n)的时间来找寻匹配项,最后时间复杂度为O(n)
根据题目我们可以得到以下信息：
...
解题思路： 这道题不是很难，是leetcode最开始的题目，要求很明确，很直接，如果我们用两个for循环，
O(n2)的时间复杂度去求解的话，很容易计算出来，但这明显不是面试官需要的答案。brute force只有在你不知道如何优化题目的时候，将就的给出的一个解法。。那么我们能不能用O(n)的时间复杂度去解这道题呢？很显然是可以的，不过，天下没有掉馅饼的事情啦，既然优化了时间复杂度，我们就要牺牲空间复杂度啦。在这里用什么呢？stack？queue？vector？还是hash_map?
